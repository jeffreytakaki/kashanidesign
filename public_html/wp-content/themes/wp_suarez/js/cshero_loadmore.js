jQuery(document).ready(function($) {
	...
	$.loadData = function(){
		...
		$.get(nextLink,function(data){
			...
			if(cs_more_obj.masonry=='masonry'){
				...
				$(data).find('#main > .blog-magazine-inner > article').each(function(){
					html += $('<div>').append($(this).clone()).html();
				});
	...
	if(pageNum <= max) {
		...
		$('.cs_pagination').append('<div class="cshero-placeholder-'+ pageNum +'"></div><p id="cshero-load-posts"><a data-loading="0" href="#">'+text+'</a></p>');
