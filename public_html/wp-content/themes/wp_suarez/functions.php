...
add_action( 'after_setup_theme', 'cshero_setup' );

function cshero_setup() {
...
function cshero_generate_dynamic_css(){
    ...
    if(count($smof_data) > 1){
        ...
        file_put_contents(get_template_directory().'/css/' . 'dynamic.css', cshero_compressCss($css), LOCK_EX); // Save it
    }
}
...
function cshero_js(){
	...
	if($smof_data['smooth_scroll'] == '1'){
        wp_register_script( 'smoothscroll', get_template_directory_uri().'/js/cs-smoothscroll.min.js', array(), '1.0', TRUE);
        wp_enqueue_script( 'smoothscroll' );
    }
    wp_register_script('widget_cart_search_scripts', get_template_directory_uri() . '/framework/widgets/widgets.js', array(), '1.0', TRUE);
	/* load base script */
	wp_enqueue_script("jquery");
	wp_enqueue_script( 'bootstrap');

	wp_deregister_script( 'jquery-cookie' );
	wp_enqueue_script( 'parallax', get_template_directory_uri().'/js/cs_parallax.js', array(), '1.0', TRUE);
	wp_enqueue_script( 'jquery-cookie', get_template_directory_uri().'/js/jquery_cookie.min.js', array(), '1.0', TRUE);
    ...
    if( $smof_data['page_loader'] == '1'){
        wp_enqueue_script( 'pageloading', get_template_directory_uri().'/js/pageloading.js', array(), '1.0', TRUE);
    }
...
if(isset($smof_data['enable_hidden_sidebar']) && $smof_data['enable_hidden_sidebar']){
	add_action( 'wp_enqueue_scripts', 'cshero_meny_script' );
	function cshero_meny_script(){
		global $smof_data;
		...
		$meny_config = array();
		$meny_config['enable_hidden_hover'] = true;
...
add_image_size( 'masonry-thumb', 570 );
add_image_size( '570x430', 570, 430,true );
add_image_size( '570x385', 570, 385,true );
add_image_size( 'blog-full', 1170, 600,true );
...
function cshero_page_title_render() {
    ...
    if (is_page() && get_post_meta(get_the_ID(), 'cs_page_title_custom_text', true) != ""){
        echo esc_attr(cshero_string_limit_words(get_post_meta(get_the_ID(), 'cs_page_title_custom_text', true),$smof_data['title_bar_length']));
    } else {
        if (!is_archive()){
            if(is_search()){
                ...
                echo cshero_string_limit_words(get_the_title(),$smof_data['title_bar_length']);
            }
        } else {
            if ( is_category() ) :
            single_cat_title();
...
if (!function_exists('cshero_tp_relative_time')) {

    function cshero_tp_relative_time($a) {
...
add_action( 'wp_ajax_sample', 'cshero_prefix_ajax_sanple' );

function cshero_prefix_ajax_sanple(){
