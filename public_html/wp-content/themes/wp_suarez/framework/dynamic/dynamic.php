...
function cshero_re_render_options()
{
    ...
    if(!empty( $smof_data['background-header']['background-color'])){
        $smof_data['background-header']['background-color'] = cshero_HexToRGB($smof_data['background-header']['background-color'],$smof_data['header_transparent']);
    }
    if(!empty($smof_data['header_border_bottom_color']['color'])){
        $smof_data['header_border_bottom_color'] = cshero_HexToRGB($smof_data['header_border_bottom_color']['color'],$smof_data['header_border_bottom_color']['alpha']);
    }

    if(!empty($smof_data['header_top2_bg_color']['color'])){
        $smof_data['header_top2_bg_color'] = cshero_HexToRGB($smof_data['header_top2_bg_color']['color'],$smof_data['header_top2_bg_color']['alpha']);
    }
    
    if(!empty($smof_data['header_top2_widgets_border_color']['color'])){
        $smof_data['header_top2_widgets_border_color'] = cshero_HexToRGB($smof_data['header_top2_widgets_border_color']['color'],$smof_data['header_top2_widgets_border_color']['alpha']);
    }
    if ($page_id) {
        /**
         * background-body
        ...
        $menu_first_level_text_uppecase =  get_post_meta($page_id, 'cs_menu_first_level_text_uppecase', true);
        if($menu_first_level_text_uppecase != ''){ $smof_data['menu_first_level_text_uppecase'] = $menu_first_level_text_uppecase; }

        $arrow_parents_item_menu =  get_post_meta($page_id, 'cs_arrow_parents_item_menu', true);
        if($arrow_parents_item_menu != ''){ $smof_data['arrow_parents_item_menu'] = $arrow_parents_item_menu; }

        $enable_menu_border =  get_post_meta($page_id, 'cs_enable_menu_border', true);
        if($enable_menu_border != ''){ $smof_data['enable_menu_border'] = $enable_menu_border; }
...
function cshero_typography_render($typography = array(), $selector)
{
    ...
    <?php echo esc_attr($selector); ?>{
        ...
        letter-spacing:<?php echo esc_attr($typography['letter-spacing']); ?>;
        }
    <?php
    return ob_get_clean();
}