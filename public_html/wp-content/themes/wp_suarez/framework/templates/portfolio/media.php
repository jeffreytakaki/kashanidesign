...
<?php
global $portfolio_category,$portfolio_gallery, $gallery_layout, $gallery_item, $portfolio_layout;
?>
<?php 
...
if($portfolio_gallery){
	if($gallery_layout == 'grid'){
		...
		echo '<div class="single-post-gallery single-post-thumbnail single-portfolio-thumbnail">';
			if (has_post_thumbnail() && ! post_password_required() && ! is_attachment() && wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'full', false) && ($portfolio_layout != 'gallery')) {
					echo '<div class="single-post-feature-img">';
					the_post_thumbnail('');
					echo '</div>';
			}
